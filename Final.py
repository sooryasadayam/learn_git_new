...
def ds_dt(t, S, PARAMS):
    ...
    # formula parsing RXN
    rxn_values = np.reshape(S, (
        24,))  #### This is where the S_vec goes,There's an adhoc Jugad of size changing here
    ...
    for i in range(0, Names_Rcnt.shape[0]):
        if ('*' in Names_Rcnt[i, 0]):
            rcnt_values[i, 0] = Rxn_dict[Names_Rcnt[i, 0].split('*')[0]] * Rxn_dict[
                Names_Rcnt[i, 0].split('*')[1]]
        else:
...
class my_problem(base):
    ...
    def _objfun_impl(self, Params):
        ...
        r.set_f_params(np.hstack((np.array([0]), Params)))  #32 params
        #r.set_f_params(Params)
        solution = np.empty((25, 1))
        T_max = 3.0
        dt = 0.1
        ...
        sse=np.sum((10**-4)*(error ** 2))
        return (sse, )
    def human_readable_extra(self):
        return "\n\t Problem dimension: " + str(self.__dim)
prob = my_problem(dim=32)  # Create a 10-dimensional problem
