...
class my_problem(base):
    ...
    def _objfun_impl(self, Params):
        ...
        solution = np.empty((2, 1))
        T_max = 3.0
        dt = 0.1
        while r.successful() and r.t < T_max - dt:
            solution = np.hstack((solution, np.vstack((np.array([r.t + dt]), r.integrate(r.t + dt)))))
        error = 2 * np.exp(-1*np.linspace(0.0, 2.9, 30)) - np.reshape((solution[1, :]),30)
        sse=np.sum(error ** 2)
        return (sse, )
    def human_readable_extra(self):
        return "\n\t Problem dimension: " + str(self.__dim)
prob = my_problem(dim=1)  # Create a 10-dimensional problem
